...
class PTTViewModel extends ChangeNotifier {
  ...
  PTTState _state = PTTState(isRecording: false, audioChunks: [], username: 'starchykov');
  ...
  void _initialize() async {
    ...
    _socket.on('audioMessage', (data) async {
      ...
      flutterSound.thePlayer.startPlayer(
          fromDataBuffer: _state.audioToPlay,
          codec: Codec.pcm16,
          whenFinished: () => flutterSound.thePlayer.closePlayer());
    });
  }
  ...
  void _createSocketConnection() {
    _socket = IO.io(
      'http://10.0.2.2:3001/',
      IO.OptionBuilder().setTransports(['websocket']).setQuery({'username': _state.username}).build(),
    );
  }
